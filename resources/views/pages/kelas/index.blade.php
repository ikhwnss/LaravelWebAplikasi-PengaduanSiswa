...
<div class="card">
        <div class="card-header">
            ...
            <div class="card-header-action ml-auto">
                <a class="btn btn-icon btn-success" href="{{ route('kelas.tambah') ?? null}}"><i class="fas fa-plus"></i></a>
            </div>
        </div>
        <div class="card-body">
                    ...
                    <tbody>
                        ...
                        @foreach ($data as $item)
                            <tr>
                                ...
                                <td>{{$item->detail}}</td>
                                <td>
                                    <a href="{{route('kelas.detail', ['id' => $item->id])}}" class="btn btn-sm btn-success">
                                        <i class="fa fa-eye"></i>
                                    </a>
                                </td>
                                <td>
                                    <div class="btn-group mb-3" role="group" aria-label="Aksi">
                                        <a href="{{route('kelas.edit',['id' => $item->id])}}" class="btn btn-sm btn-warning">
                                            <i class="fa fa-edit"></i>
                                        </a>
                                        <form action="{{route('kelas.delete',['id' => $item->id])}}" method="POST">
                                                @csrf
                                                @method('DELETE')
