...
@section('content')
<div class="card">
    <form method="POST" action="{{route('siswa.update', ['id' => $data->id])}}">
        @csrf
        @method('POST')
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                <label>NIS</label>
                <input type="text" name="nis" class="form-control @error('nis') is-invalid @enderror" required="" value="{{ old('nis') ?? $data->nis ?? null}}">
                @error('nis')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
