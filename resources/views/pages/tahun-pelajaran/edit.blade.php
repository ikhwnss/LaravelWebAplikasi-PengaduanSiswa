...
@section('content')
<div class="card">
    <form method="POST" action="{{route('tahun-pelajaran.update', ['id' => $data->id])}}">
        @csrf
        @method('POST')
        ...
        <div class="card-body">
            <div class="form-group">
                <label>Nama</label>
                <input type="text" name="nama" class="form-control @error('nama') is-invalid @enderror" required="" value="{{ old('nama') ?? $data->nama ?? null }}" value="{{ old('nominal') ?? $data->nominal ?? null}}">
                @error('nama')
