...
@section('content')
<div class="card">
    <form method="POST" action="{{route('guru.update', ['id' => $data->id])}}">
        @csrf
        @method('POST')
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <input type="text" name="kode_guru" class="form-control @error('kode_guru') is-invalid @enderror" required="" value="{{ old('kode_guru') ?? $data->nip ?? null}}">
                @error('kode_guru')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="alamat_rumah" id="alamat_rumah @error('alamat_rumah') is-invalid @enderror" cols="30" rows="10" class="form-control">{{ old('alamat_rumah') ?? $data->alamat_rumah ?? null}}</textarea>
                @error('alamat_rumah')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
